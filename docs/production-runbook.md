...
Wallet passes refresh directly when a customer adds or restores a pass and after staff confirms a transaction. The durable Wallet outbox is retained for operational recovery, but it is not scheduled on Vercel Hobby; retry a failed pass from the customer or staff flow. A per-minute background worker can be restored later on Vercel Pro or another job runner.

Vercel calls `GET /api/maintenance` daily at 04:17 UTC to expire temporary records and clean bounded operational tables. It requires `Authorization: Bearer <CRON_SECRET>`; Vercel adds this header automatically when the project has a `CRON_SECRET` environment variable. Cron jobs run only for Production deployments, use UTC, may be delivered more than once, and are not retried by Vercel after a failed invocation.

After the first production deployment, open Vercel Project Settings > Cron Jobs and verify the daily maintenance schedule exists. Inspect its first successful invocation without logging its authorization header. If an Instant Rollback is used, check the Cron Jobs page separately because rolling back an application deployment does not roll back active cron configuration.
